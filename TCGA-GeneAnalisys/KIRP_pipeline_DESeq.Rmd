---
title: "Pipeline of Differential Expression Analysis of KIRP data"
output: 
  html_document: 
    default
  github_document: 
...
ddsObj <- DESeqDataSetFromMatrix(countData = as.matrix(kirp.count[,codes.type]),
                              colData = kirp.clini[codes.type,],
...
dea.NT.TP <- as.data.frame(res.shr) %>%
    ...
    left_join(kirp.annot, by="symbol") %>% 
...
save(dea.NT.TP, file = "dea.NT.TP.rda", compress = T)
...
glXYPlot(
  ...
  groups = kirp.clini[codes.type, "sample.type"],
  side.xlab = "Group",
  side.ylab = "Expression (log2)",
  samples = kirp.clini[codes.type, "codes"],
  status = de,
  side.main = "symbol",
...
# create the DESeqDataSet object
ddsObj <- DESeqDataSetFromMatrix(countData = as.matrix(kirp.count[,codes.metas]),
                              colData = kirp.clini[codes.metas,],
                              design = ~ metastasis)
...
save(dea.M0.M1, file = "dea.M0.M1.rda", compress = T)
...
glXYPlot(
  ...
  main = "M0.vs.M1",
  counts = counts(ddsObj)[filtTab.deseq$symbol,] ,
  groups = kirp.clini[codes.metas, "metastasis"],
  side.xlab = "Group",
  side.ylab = "Expression",
  samples = kirp.clini[codes.metas, "codes"],
  status = de,
  side.main = "symbol",
  display.columns = c("symbol", "logFC", "FDR", "ensgene", "description"),
  anno = filtTab.deseq,
  folder = "XYPlot_KIRP.M0.vs.M1",
  html = "index",
  launch = F)
```
